...
未掲載のローカル配信枠がmiの一覧に混ざるとの報告を受け、依頼を0yp限定と誤解し、本番のHTTP番組一覧取得先を0ypの `https://yayaue.me/yp/index.txt` のみに変更した。SP・p@YPのPCP接続先は維持し、両者の `channels_url` を削除した。mi側ではYP未掲載のローカルチャンネル追加を廃止する変更を用意した。TOMLの構文と一覧URLが0ypのみであることをローカル検証。本番適用は未実施。

### 一覧取得先の解釈訂正（2026-09-14）

ユーザーが、3YPの取得は維持しYP未掲載のローカル配信枠だけを除外する意図だったと確認した。SP・p@YPのchannels_urlを復元し、0yp・SP・p@YPの順で取得する本番設定へ戻した。掲載先は0yp固定、miのローカル枠除外は維持。TOMLパースと3件の名前・URL・順序をローカル検証済み。本番反映はAnsible適用待ち。

### miの公開アドレス設定（2026-09-14）

0ypの公開APIでいまいchのtrackerがDocker内部172.20.0.4:7154になっていた。DNSでyayaue.meのIPv4を153.127.50.98と確認し、外部から7154へのTCP接続も成功。miの新設定public_ipv4に153.127.50.98を指定し、内部YPのOLEH観測値より公開HOST欄で優先する。内外のPCPポートは7154で一致。TOMLパース確認済み。アプリ更新後のAnsible適用が必要で、本番掲載の更新と外部PCP再生は未確認。VPSのIP変更時にはこの値も更新する。

### セッション終了時のユーザー確認（2026-09-14）
